...
\begin{frame}{What We Will Learn in This Chapter}
\small
\begin{itemize}
...
\centering
\includegraphics[width=0.65\linewidth]{Figure/FlorentinFlorin.jpg}\\[0.3cm]
...
\centering
\includegraphics[width=0.7\linewidth]{Figure/GoldDollar.png}\\[0.4cm]
...
\centering
\includegraphics[width=0.7\linewidth]{Figure/Dollar.jpg}\\[0.4cm]
\end{frame}
...
\includegraphics[height=0.78\textheight,keepaspectratio]{Figure/Pounds.jpg}\\[0.1cm]
...
\begin{itemize}
  \item The central bank (CB) sets the interest rate on reserves, denoted by $i_t^X$.
...
\begin{itemize}
  \item Cash and bonds are \textbf{perfect substitutes} as media of exchange 
  (see budget constraint~(\ref{eq: budget})).
  \item Bonds pay a non-negative interest rate, while cash yields zero.
  \item $\Rightarrow$ When $i_t>0$, bonds dominate cash, and money demand is zero.
...
\begin{itemize}
  ...
  \item Moreover, their debt—carrying a default-free interest rate $i_t$—must be repaid with certainty.
\end{itemize}
...
\begin{itemize}
  \item In equation~(\ref{eq:nom_borrowing}), debt cannot exceed available assets and the PDV of net income.
\end{itemize}
...
\begin{itemize}
    \item The nominal borrowing limit in \eqref{eq:nom_borrowing} can be expressed in real terms:
\end{itemize}
...
\begin{itemize} \item Note, that the real interest rate $r$ is defined as $1+r_t \equiv (1+i_t)\frac{P_t}{P_{t+1}}$. \end{itemize}
...
\begin{itemize}
  \item The borrowing limit in~(\ref{eq:real_borrowing}) is \textbf{equivalent} to the household’s intertemporal budget constraint, using eq. (\ref{eq: budget}).
\end{itemize}
...
\begin{itemize}
  \item The present discounted value (PDV) of consumption cannot exceed:
  \begin{itemize}
...
\begin{itemize}
    \item Constraints  \eqref{eq:real_borrowing} or \eqref{eq:borrowing} can be equivalently expressed, using eq. (\ref{eq: budget}), as an asset limit constraint.
\end{itemize}
...
\vspace{0.3cm}
\begin{center}
\hyperlink{limit_proof}{\beamerbutton{Proof available in Appendix}}
\end{center}
...
\hspace{0.6cm}where $U_c(C_t)$ denotes the marginal utility of consumption at time~$t$.
...
\begin{itemize}
    \item At the optimum, consumers exhaust all resources $\longrightarrow$ the intertemporal budget constraint holds with equality.
\end{itemize}
...
\vspace{0.25cm}
\begin{itemize}
    \item Or, equivalently, the transversality condition holds, i.e. eq. (\eqref{Blimit}) with equality:
\end{itemize}
\noindent\textbf{Transversality Condition:}
\begin{equation}
...
\begin{itemize}
    \item Reserves $X^{C}$ are claims on themselves; they can \textbf{always} be paid, even if $B_t^C = 0$ $\longrightarrow$ \textbf{No} solvency condition is required on $X^{C}$. 
    \vspace{0.25cm}
...
\begin{itemize}
    \item The Treasury issues debt. At time $t$ must pay its outstanding obligations $B_{t-1}^{F}$ by:
    \begin{itemize}
...
\begin{itemize}
    \item $B_{t-1}^{F}$ are claims on currency, \textbf{not} currency. They are not, by definition, default-free!
    \vspace{0.25cm}
    \item To be default free, Treasury debt has to satisfy a solvency constraint.
\end{itemize}
...
\noindent or equivalently \textbf{Limit Condition:} 
...
\begin{frame}[label=gdp_exp]{Market Clearing Conditions}
...
\begin{itemize}
  \item Substituting $C_t = Y$ into the Euler equation \eqref{eq:euler} delivers the Fisher equation:
\end{itemize}

\vspace{0.25cm}
\noindent\textbf{Fisher Equation:}
\begin{equation}
\underbrace{1 + i_t}_{\text{\scriptsize nominal rate}}
\;=\;
\underbrace{\frac{1}{\beta}}_{\text{\scriptsize real rate}}
\;\underbrace{\frac{P_{t+1}}{P_t}}_{\text{\scriptsize inflation}},
\label{eq: Fisher}
\end{equation}


\begin{itemize}
  \item This condition links the nominal interest rate to the real rate and to next period inflation.
  \item Note: with a constant endowment, the real interest rate is constant and equal to $1/\beta$.
\end{itemize}
\end{frame}

\begin{frame}{Equilibrium Condition: Intertemporal Resource Constraint}
\begin{itemize}
  \item Using $C_t=Y$ and the Fisher equation \eqref{eq: Fisher}, the intertemporal budget constraint \eqref{eq: IBCCE} can be written as the:
\end{itemize}
\noindent\textbf{Intertemporal Resource Constraint}
\vspace{0.25cm}
\begin{equation}
  \frac{B_{t_{0}-1} + X_{t_{0}-1}}{P_{t_{0}}}
  = \sum_{t=t_{0}}^{\infty} \beta^{\,t-t_{0}} \frac{T_t}{P_t}
  \label{eq: IBCTT}
\end{equation}

\vspace{0.25cm}
\begin{itemize}
  \item \textbf{LHS:} Real net government liabilities to the private sector at $t_0$ 
        (including both Treasury debt and CB liabilities).
  \item \textbf{RHS:} Present-discounted value of real taxes collected.
  \item The intertemporal resource constraint is not a solvency constraint but an \textbf{equilibrium condition}.  
\end{itemize}
\end{frame}

\begin{frame}{Equilibrium Condition: Treasury Solvency}
Treasury’s solvency condition is given by \eqref{DebtBB}, stated with equality and using the Fisher equation \eqref{eq: Fisher}:
...
\begin{equation}
  ...
  = \sum_{t=t_0}^{\infty} \beta^{\,t-t_0}
    ...
    \label{eq: Treasury Solvency}
\end{equation}
\vspace{0.35cm}
\break
Treasury solvency requires that the stock of outstanding real debt be backed by the present discounted value of real taxes at any equilibrium path of prices and CB remittances.
\end{frame}

\begin{frame}{Equilibrium Conditions: Budget Constraints}

\vspace{0.35cm}
\noindent\textbf{CB Budget Constraint in Equilibrium:}
\begin{equation}
\frac{X_{t}-B_{t}^{C}}{1+i_{t}} = X_{t-1} - B_{t-1}^{C} + T_{t}^{C} 
\label{FlowG}
\end{equation}
\vspace{0.5cm}

\noindent\textbf{Treasury Budget Constraint in Equilibrium:}
\begin{equation}
\frac{B_{t}^{F}}{1+i_{t}} = B_{t-1}^{F} - T_{t} - T_{t}^{C}  
\label{TBC}
\end{equation}
...
\]
with $\{ P_t, i_t, X_t, B_t^C \}_{t=t_0}^\infty \geq 0$ that satisfies:
\begin{itemize}
  \item Bond market clearing (\ref{eq: Bonds})
  \item Euler equation (\ref{eq: Fisher})
  \item Intertemporal resource constraint (\ref{eq: IBCTT})
  \item Government budget constraints (\ref{FlowG}), (\ref{TBC})
\end{itemize}

given initial conditions $B_{t_0-1}^C, B_{t_0-1}^F, X_{t_0-1}$.  
Taxes $\{T_t\}_{t=t_0}^\infty$ must satisfy the solvency condition (\ref{eq: Treasury Solvency}) for any equilibrium sequences $\{P_t,T_t^C\}_{t=t_0}^\infty$, given the initial condition $B_{t_0-1}^F$ .
\end{frame}
\begin{frame}{Government Policy}
\begin{itemize}
\item Equilibrium characterization shows 4 degrees of freedom, which are the policy choices that the government can take.
  \vspace{0.3cm}
  \item CB chooses sequences $\{ i_t, X_t, T_t^C \}_{t=t_0}^\infty$, with asset holdings adjusting via (\ref{FlowG}).
\vspace{0.3cm}
\item Treasury sets taxes $\{T_t\}_{t=t_0}^\infty$ to satisfy (\ref{eq: Treasury Solvency}) at \emph{any} equilibrium sequences for prices $\{P_t\}_{t=t_0}^\infty$ and CB remittances $\{T_t^C\}_{t=t_0}^\infty.$
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item Interest-rate policy alone cannot determine the currency's value.
...
\textbf{Policy Rule}
    ...
    \end{equation}
\begin{itemize}
    \item In \eqref{eq: int_rule}, the interest rate is equal to the real rate $1/\beta$ and reacts with a coefficient $\phi$ to the ratio of the current price level with respect to the target $\bar{P}$, unless the zero lower bound is achieved.
\end{itemize}
\end{frame}
...
\IfFileExists{Figure/Figure2.png}{%
    \includegraphics[height=0.7\textheight,keepaspectratio]{Figure/Figure2.png}%
}{%
...
\begin{itemize}
    ...
    \item \textbf{Distinction between Treasury and Central Bank (CB) liabilities:}
  \begin{itemize}
    \item \textbf{Treasury debt} promises payment in a currency for which the Treasury has \textbf{limited resources}.
    \item \textbf{Central bank liabilities} are redeemable in \textbf{units of themselves}, and can therefore be issued in \textbf{unlimited supply}.
  \end{itemize}
    \vspace{0.15cm}    
    \item In FTPL the CB \textbf{fully guarantees} treasury debt. Distinction between treasury and CB is now blurred.
    \item Solvency condition \eqref{DebtBB} no longer constraints the path of taxes.
    \vspace{0.15cm}
    \item The government budget constraints are now consolidated, by using \eqref{CBBC1}, \eqref{BudgetT} and bond market clearing \eqref{eq: Bonds}:
    \end{itemize}
...
Given that tax policy is no longer constrained by solvency condition \eqref{eq: Treasury Solvency}, equilibrium is characterized by a set of sequences:
\[
...
\begin{itemize}
    \item The key factor in influencing the price level is the formulation of the tax policy.
    \vspace{0.25cm}
\item Consider the constant real tax policy:
    ...
    \end{equation}
    \vspace{0.25cm}
    \item Substituting into the intertemporal budget condition (\ref{eq: IBCTT}) gives:
...
\begin{itemize}
    ...
    \vspace{0.5cm}
\item This type of policy is called \textbf{active} because taxes are fixed in real terms and do not adjust to ensure solvency. Indeed, solvency is \textbf{guaranteed} by the central bank.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Central Bank (CB) backing:}  
...
\begin{itemize}
  ...
  \item Unlike the FTPL, the CBTPL relies on the central bank (CB) to \textbf{independently determine the price level}.
...
\begin{itemize}
    ...
    \end{equation}
    \vspace{0.25cm}
    \item Price is positive if $B_{t_0-1}^C > X_{t_0-1}^C$, i.e., CB starts with positive net worth.
    \item Setting $\tau^C = (B_{t_0-1}^C - X_{t_0-1}) / \bar P$ pins $P_{t_0} = \bar P$, independent of $\phi$ in (\ref{eq: int_rule}).
    \vspace{0.25cm}
...
\begin{itemize}
  ...
  behind \textbf{price determination}.
...
\begin{itemize}
    ...
    \vspace{0.25cm}
    \item Rewriting the CB's budget constraint \eqref{CBBC1} using the definition of net worth, $N_t^C$, via eq. \eqref{eq: NW} :
    \[
    N_t^C = N_{t-1}^C + \Psi_t^C - T_t^C, 
    \quad \Psi_t^C = i_{t-1} N_{t-1}^C
    \]
    \vspace{0.25cm}
...
\begin{itemize}
  ...
  \vspace{0.2cm}
  \item Without coordination, the \textbf{central bank (CB)} can provide real backing through a policy of distributing real earnings, which requires:
  \begin{itemize}
  ...
  \vspace{0.2cm}
  \item The \textbf{CBTPL} therefore shows how the CB alone can \textbf{anchor the price level}, provided it begins with \textbf{positive net worth}.
\end{itemize}
...
\item Consider the case in which the CB does not back the treasury.
    \item Starting from the interest-rate rule (\ref{eq: int_rule}) and equilibrium in goods, asset, and gold markets, together with the treasury solvency condition (\ref{eq: Treasury Solvency}) and $R_{t_0,t} = \beta^{t-t_0}$, we obtain using \eqref{eq: IBC_gold}:
\end{itemize}
...
Let $\lambda_t$ be the multiplier on \eqref{eq:budget} and $\tilde{\zeta}_t \ge 0$ the multiplier on \eqref{eq:borrowing}.
